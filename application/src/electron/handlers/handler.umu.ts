...
import {
  ...
  getUmuPrefixBaseIn,
  getUmuWinePrefixIn,
  normalizeProtonPathValue,
  parseLaunchArguments,
  parseLeadingLaunchEnvFromArguments,
...
/**
 * Get the home directory UMU prefixes live under
 * Throws an error if home directory cannot be determined
 */
function getUmuPrefixHome(): string {
...
export {
  ...
  getEffectiveDllOverrides,
  getEffectiveLaunchEnv,
  inferDllOverridesFromLaunchArguments,
  parseLaunchArguments,
...
export function getUmuWinePrefix(gameId: string): string {
  return getUmuWinePrefixIn(getUmuPrefixHome(), gameId);
}

export function getLibraryUmuWinePrefix(
  libraryInfo: Pick<LibraryInfo, 'umu'>
): string {
  return getLibraryUmuWinePrefixIn(getUmuPrefixHome(), libraryInfo);
}
